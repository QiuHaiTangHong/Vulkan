...
module CustomVulkan.Sync;

namespace CustomVulkan {
    template <typename Tag>
    [[nodiscard]] auto VulkanSyncObjects<Tag>::createVulkanSyncObjects() const -> std::shared_ptr<GlfwContext>
        requires std::is_same_v<Tag, IVulkanInit>
    {
        ...
        try {
            ...
            ctx->vulkanContext.imageAvailableSemaphores = VulkanResource<std::vector<vk::Semaphore>>(
                imgSems, [device](const std::vector<vk::Semaphore>& semaphores) {
            ...
            ctx->vulkanContext.renderFinishedSemaphores = VulkanResource<std::vector<vk::Semaphore>>(
                renSems, [device](const std::vector<vk::Semaphore>& semaphores) {
                    for (const auto& semaphore : semaphores) {
                        device.destroySemaphore(semaphore);
                    }
                    std::cout << "[Vulkan 销毁信息]: 销毁渲染完成信号量(renderFinishedSemaphore)!\n";
                });
            ctx->vulkanContext.inFlightFences =
                VulkanResource<std::vector<vk::Fence>>(fences, [device](const std::vector<vk::Fence>& fs) {
