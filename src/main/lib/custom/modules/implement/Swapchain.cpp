...
module CustomVulkan.Swapchain;

namespace CustomVulkan {
    template <typename Tag>
    [[nodiscard]] auto VulkanImageViews<Tag>::createVulkanImageViews() const -> VulkanRenderPass<IVulkanInit>
        requires std::is_same_v<Tag, IVulkanInit>
    {
        initVulkanImageViews(this->ctx);
        return {ctx};
    }

    template <typename Tag>
    [[nodiscard]] auto VulkanImageViews<Tag>::recreateVulkanImageViews() const -> VulkanRenderPass<IVulkanRecreate>
    ...
    template <typename Tag>
    void VulkanImageViews<Tag>::initVulkanImageViews(const std::shared_ptr<GlfwContext>& ctx) {
        ...
        try {
            ...
            ctx->vulkanContext.swapChainImageViews = VulkanResource<std::vector<vk::ImageView>>(
                std::move(tempImageViews), [device](const std::vector<vk::ImageView>& views) {
                    for (auto v : views) {
                        if (v) {
    ...
    template <typename Tag>
    [[nodiscard]] auto VulkanSwapChain<Tag>::createVulkanSwapChain() const -> VulkanImageViews<IVulkanInit>
        requires std::is_same_v<Tag, IVulkanInit>
    {
        initSwapChain(this->ctx);
        return {ctx};
    }

    template <typename Tag>
    [[nodiscard]] auto VulkanSwapChain<Tag>::recreateSwapChain() const -> VulkanImageViews<IVulkanRecreate>
        requires std::is_same_v<Tag, IVulkanRecreate>
    {
        ctx->vulkanContext.swapChainFramebuffers.cleanup();
        ctx->vulkanContext.commandBuffer.cleanup();
        ctx->vulkanContext.graphicsPipeline.cleanup();
        ctx->vulkanContext.pipelineLayout.cleanup();
        ctx->vulkanContext.renderPass.cleanup();
        ctx->vulkanContext.swapChainImageViews.cleanup();
        ctx->vulkanContext.swapChain.cleanup();
        initSwapChain(this->ctx);
        return {ctx};
    }

    template <typename Tag>
    void VulkanSwapChain<Tag>::initSwapChain(const std::shared_ptr<GlfwContext>& ctx) {
        auto [capabilities, formats, presentModes] =
            querySwapChainSupport(ctx->vulkanContext.physicalDevice, ctx->vulkanContext.surface.get());
        ...
        createInfo.setSurface(ctx->vulkanContext.surface.get())
        ...
        try {
            ...
            ctx->vulkanContext.swapChain = VulkanResource<vk::SwapchainKHR>(
                device.createSwapchainKHR(createInfo), [device](const vk::SwapchainKHR s) {
                    if (s) {
    ...
    // 查询交换链支持
    template <typename Tag>
    SwapChainSupportDetails VulkanSwapChain<Tag>::querySwapChainSupport(const vk::PhysicalDevice device,
                                                                        const vk::SurfaceKHR surface) {
        return SwapChainSupportDetails{device.getSurfaceCapabilitiesKHR(surface), device.getSurfaceFormatsKHR(surface),
    ...
    template <typename Tag>
    vk::SurfaceFormatKHR
    VulkanSwapChain<Tag>::chooseSwapSurfaceFormat(const std::vector<vk::SurfaceFormatKHR>& availableFormats) {
    ...
    template <typename Tag>
    vk::PresentModeKHR
    VulkanSwapChain<Tag>::chooseSwapPresentMode(const std::vector<vk::PresentModeKHR>& availablePresentModes) {
    ...
    // 选择交换链范围
    template <typename Tag>
    vk::Extent2D VulkanSwapChain<Tag>::chooseSwapExtent(const vk::SurfaceCapabilitiesKHR& capabilities,
                                                        GLFWwindow* window) {
    ...
    template struct VulkanSwapChain<IVulkanInit>;
    template struct VulkanSwapChain<IVulkanRecreate>;

    template struct VulkanImageViews<IVulkanInit>;
    template struct VulkanImageViews<IVulkanRecreate>;
