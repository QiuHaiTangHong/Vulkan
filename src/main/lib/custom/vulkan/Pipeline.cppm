...
export namespace CustomVulkan {
    ...
    struct VulkanGraphicsPipeline {
        ...
        [[nodiscard]] auto
        createVulkanGraphicsPipeline() const -> VulkanFramebuffers<IVulkanInit> requires std::is_same_v<
            Tag, IVulkanInit> {
        ...
        [[nodiscard]] auto
        recreateVulkanGraphicsPipeline() const -> VulkanFramebuffers<IVulkanRecreate> requires std::is_same_v<
            Tag, IVulkanRecreate> {
            initVulkanGraphicsPipeline(this->ctx);
            return {ctx};
        }
        ...
        private:
            ...
            static auto createShaderModule(
                ...
                try {
                    ...
                    std::cerr << "[Vulkan 错误信息]: ShaderModule 创建失败: " << err.
                            what() << std::endl;
                }
                return {handle,
    ...
    struct VulkanRenderPass {
        ...
        [[nodiscard]] auto
        createVulkanRenderPass() const -> VulkanGraphicsPipeline<IVulkanInit> requires std::is_same_v<
            Tag, IVulkanInit> {
        ...
        [[nodiscard]] auto
        recreateVulkanRenderPass() const -> VulkanGraphicsPipeline<IVulkanRecreate> requires std::is_same_v<
            Tag, IVulkanRecreate> {
            initVulkanRenderPass(this->ctx);
            return {ctx};
        }
