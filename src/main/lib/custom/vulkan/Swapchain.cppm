...
export namespace CustomVulkan {
    ...
    struct VulkanImageViews {
        ...
        [[nodiscard]] auto createVulkanImageViews() const -> VulkanRenderPass<IVulkanInit> requires std::is_same_v<
            Tag, IVulkanInit> {
            initVulkanImageViews(this->ctx);
            return {ctx};
        }

        [[nodiscard]] auto recreateVulkanImageViews() const -> VulkanRenderPass<IVulkanRecreate> requires std::is_same_v
            <Tag, IVulkanRecreate> {
            initVulkanImageViews(this->ctx);
            return {ctx};
        }
    ...
    struct VulkanSwapChain {
        ...
        [[nodiscard]] auto createVulkanSwapChain() const -> VulkanImageViews<IVulkanInit> requires std::is_same_v<
            Tag, IVulkanInit> {
            initSwapChain(this->ctx);
            return {ctx};
        }

        [[nodiscard]] auto recreateSwapChain() const -> VulkanImageViews<IVulkanRecreate> requires std::is_same_v<
            Tag, IVulkanRecreate> {
        ...
        private:
            ...
            static void initSwapChain(const std::shared_ptr<GlfwContext> &ctx) {
                ...
                try {
                    ...
                    ctx->vulkanContext.swapChainImages = device.getSwapchainImagesKHR(
                            ctx->vulkanContext.swapChain.get()
                            );
